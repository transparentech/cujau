...
public class ResourceUtil {
     ...
     * @return A String containing the requested resource or <tt>null</tt> if no resource with the give name is
     * available.
     * @throws IOException
     */
    ...
    /**
     * Return the requested resource as a String. If the resource starts with "file:", it will be loaded from the file
     * system, otherwise it will be loaded as a resource.
     * <p>
     * This method assumes that the resource is a text file encoded in UTF-8.
     * </p>
     *
    ...
    public static String getResourceAsString(Class<?> klass, String resourceName)
        ...
        if (resourceName.startsWith("classpath:")) {
            return doGetResourceAsString(klass, resourceName.substring(10));
        } else if (resourceName.startsWith("file:")) {
            return FileUtil.getFileAsString(new File(resourceName.substring(5)));
        } else {
            return doGetResourceAsString(klass, resourceName);
        }
    }

    private static String doGetResourceAsString(Class<?> klass, String resourceName)
            throws IOException {
     ...
     * @param resourceName
     *         The resource on the classpath.
     * @return An arra of bytes containing the requested resource or <tt>null</tt> if no resource with the given name is
     * available.
     * @throws IOException
     */
    ...
    /**
     * Return a File representing the location on disk of the given class. For a class contained in a JAR file, the
     * returned File will be the containing JAR file. For a class in a directory, the returned File will be the
     * directory on the classpath that contains (likely via a sub-directory) the class.
     *
     * @param klass
